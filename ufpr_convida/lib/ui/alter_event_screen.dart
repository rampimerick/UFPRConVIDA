...
class _alterEventState extends State<alterEvent> {
  ...
  Widget build(BuildContext context) {
    ...
    _eventTargetController.value = _eventTargetController.value.copyWith(
        text: event.target,
        selection: TextSelection(baseOffset: event.target.length, extentOffset: event.target.length),
        composing: TextRange.empty
    );
    //Desc
    _eventDescController.value = _eventDescController.value.copyWith(
        text: event.desc,
        selection: TextSelection(baseOffset: event.desc.length, extentOffset: event.desc.length),
        composing: TextRange.empty
    );
    //Link
    _eventLinkController.value = _eventLinkController.value.copyWith(
        text: event.link,
        selection: TextSelection(baseOffset: event.link.length, extentOffset: event.link.length),
        composing: TextRange.empty
    );
    //Type
    _eventTypeController.value = _eventTypeController.value.copyWith(
        text: event.type,
        selection: TextSelection(baseOffset: event.type.length, extentOffset: event.type.length),
        composing: TextRange.empty
    );
    //Sector
    //Block
  ...
  void name() {
    _eventNameController.value = _eventNameController.value.copyWith(
      text: event.name,
      selection: TextSelection(baseOffset: event.name.length, extentOffset: event.name.length),
      composing: TextRange.empty,
    );
  }
}
